...
// ham con cua task 2: no nhan ID tu server qua stream, kiem tra xem ID do:
// 1. trung voi ID hien tai -> giu nguyen tat ca trang thai
// 2. ID duoc thay doi -> thay the IdNguoiGuiTruoc, id trong eeprom
// 3. ID rong -> xoa tat ca ID
// chi khi dong tu ID moi duoc chap nhan chinh sua
...
void quanLyCuaVaRFIDTask(void *pvParameters)
{
  ...
  bool capNhatTrangThaiCua = true;    // giup cho ham nay chi chay mot lan khi dieu kien cua dong/ mo bi lap lai lien tuc
  String savedId = "";
  ...
  for (;;)
  {
    ...
    if (!daKhoiTaoEEPROM)
    {
      ...
      // Đánh dấu là đã chạy xong, bien nay nham giu cho vong nay chi chay lan dau tien khi chuong trinh khoi tao
    ...
    trangThaiCua = digitalRead(congTacHanhTrinh);
    if (trangThaiCua == LOW)
    {
      if (capNhatTrangThaiCua == true)
      {
        Serial.println("Cua dang dong.");
        // gui trang thai cua cho task 3
        xQueueSend(stateDoorQueue, &trangThaiCua, 100);
        capNhatTrangThaiCua = false;
      }
      ...
      if ((IdNguoiGuiTruoc == "") && rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial())
      {
        ...
        trangThaiCua = true;                          // Cửa đã được mở
        xQueueSend(stateDoorQueue, &trangThaiCua, 0); // Gửi trạng thái cửa cho Task 3
        vTaskDelay(200 / portTICK_PERIOD_MS);         // Cho Queue có thời gian xử lý
        ...
        // Cập nhật trạng thái cửa
        trangThaiCua = false;                         // Cửa đã được đóng
        xQueueSend(stateDoorQueue, &trangThaiCua, 0); // Gửi trạng thái cửa cho Task 3
        vTaskDelay(200 / portTICK_PERIOD_MS);         // Cho Queue có thời gian xử lý
      ...
      else if ((IdNguoiGuiTruoc != "") && rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial())
      {
        ...
        else
        {
          ...
          // trang thai led
          bool statusLedDo = digitalRead(ledDo);
          bool statusLedXanh = digitalRead(ledXanh);
          // chop tat led do
          digitalWrite(ledXanh, LOW);
          digitalWrite(ledDo, HIGH);
          ...
          vTaskDelay(200 / portTICK_PERIOD_MS);
          digitalWrite(ledDo, HIGH);
          // tra lai trang thai led
          digitalWrite(ledXanh, statusLedXanh);
          digitalWrite(ledDo, statusLedDo);
        }
        rfid.PICC_HaltA();
        rfid.PCD_StopCrypto1();
      }
    ...
    { // Mức 1 -> Cửa đang mở
      digitalWrite(khoaDien, LOW);
      if (capNhatTrangThaiCua == false)
      {
        Serial.println("Cua dang mo.");
        // gui trang thai cua cho task 3
        xQueueSend(stateDoorQueue, &trangThaiCua, 0);
        capNhatTrangThaiCua = true;
      }
    }
...
/**
 * @brief TASK 3: XỬ LÝ GIAO TIẾP FIREBASE (PHIÊN BẢN STREAM HOÀN CHỈNH)
 * - Gửi dữ liệu (trạng thái cửa, trạng thái tủ, UID) lên Firebase khi nhận từ Queue.
 * - Duy trì kết nối Stream để nhận lệnh cập nhật/xóa UID real-time (thông qua callback).
 */
 ...
 */
void guiNhanFirebaseGooleSheetTask(void *pvParameters)
{
  // lấy thời gian hệ thống, sau mỗi 30s kiểm tra currentID trên firebase một lần
  // nếu nó là một string "", nghĩa là xóa ID
  time_t now = time(nullptr);    // Lấy thời gian hiện tại, biến cục bộ, giải phóng ngay sau khi hàm kết thúc
  time_t mocThoiGian = now + 30; // Mốc 5 giây sau thời điểm hiện tại

  // biến nhận dữ liệu từ queue
  UploadData_t duLieuNhanDuoc;
  bool trangThaiCuaNhanDuoc;
  ...
  for (;;)
  {
    if (WiFi.status() == WL_CONNECTED)
    {
      ...
      // Cho den khi co du lieu trong stateDoorQueue
      if (xQueueReceive(stateDoorQueue, &trangThaiCuaNhanDuoc, 0) == pdTRUE)
      {
        // co du lieu trong stateDoorQueue
        Serial.printf("[Task 3] Nhan duoc trang thai cua: %s\n", trangThaiCuaNhanDuoc ? "Mo" : "Dong");
        // Gui len Firebase
        Serial.println("--- Dang gui trang thai cua len Firebase ---");
        // gui du lieu
        Firebase.RTDB.setString(&fbdo, "/TestTrangWeb/lockStatus", String(trangThaiCuaNhanDuoc ? "Mo" : "Dong"));
        Serial.println("Du lieu gui len: trang thai cua: " + String(trangThaiCuaNhanDuoc ? "Mo" : "Dong"));
        vTaskDelay(500 / portTICK_PERIOD_MS);
      }

      // Cho den khi co du lieu trong uploadQueue
      if (xQueueReceive(uploadQueue, &duLieuNhanDuoc, 0) == pdTRUE)
      {
        ...
        // gui du lieu
        Firebase.RTDB.setString(&fbdo, "/TestTrangWeb/itemStatus", String(duLieuNhanDuoc.coDo ? "Co do" : "Trong"));
      ...
      // ================== NHẬN LỆNH TỪ FIREBASE (POLLING MỖI 30 GIÂY) ==================

      // SỬA LỖI: Lấy thời gian 'now' bên trong vòng lặp
      time_t now = time(nullptr);
      if (now > mocThoiGian)
      {
        Serial.println("[Task 3] Da den moc 30s, kiem tra currentID tren Firebase...");

        String currentIDFirebase = "";
        if (Firebase.RTDB.getString(&fbdo, "/TestTrangWeb/currentID"))
        {
          currentIDFirebase = fbdo.stringData();
          Serial.println("currentID tren Firebase: " + currentIDFirebase);
        }
        else
        {
          Serial.printf("Loi khi doc Firebase: %s\n", fbdo.errorReason().c_str());
        }

        // Lấy ID từ EEPROM để so sánh
        String IdHienTaiTrongTu = "";
        if (xSemaphoreTake(preferencesMutex, pdMS_TO_TICKS(1000)) == pdTRUE)
        {
          preferences.begin("tuThongMinh", true);
          IdHienTaiTrongTu = preferences.getString("IdNguoiGui", "");
          preferences.end();
          xSemaphoreGive(preferencesMutex);
        }
        Serial.println("currentID trong EEPROM: " + IdHienTaiTrongTu);

        // neu EEPROM trong, ID la N/A
        if (IdHienTaiTrongTu == "")
        {
          IdHienTaiTrongTu = "N/A";
        }

        // Logic so sánh
        if (currentIDFirebase != IdHienTaiTrongTu)
        {
          Serial.println("[Task 3] Phat hien thay doi!");
          if (currentIDFirebase == "N/A")
          {
            // cap nhat ID
            if (xSemaphoreTake(preferencesMutex, portMAX_DELAY) == pdTRUE)
            {
              // Cap nhat ID vào EEPROM
              preferences.begin("tuThongMinh", false);
              preferences.putString("IdNguoiGui", "");
              preferences.end();
              Serial.println("Xoa ID");
            }
          }
          else
          {
            // Cap nhat ID vào EEPROM
            preferences.begin("tuThongMinh", false);
            preferences.putString("IdNguoiGui", currentIDFirebase);
            preferences.end();
            Serial.println("Cap nhat ID: " + currentIDFirebase);
          }
          Serial.println("ID da duoc cap nhat, khoi dong lai he thong!");
          ESP.restart(); // Khởi động lại ESP32
        }
        else
        {
          Serial.println("[Task 3] Khong co gi thay doi, ID da dong bo.");
        }
        mocThoiGian = now + 30; // Cập nhật mốc thời gian mới
      }
    ...
    {
      Serial.println("Loi: Mat ket noi internet, khong the gui du lieu.");
...
void setup()
{
  ...
  if (uploadQueue == NULL || triggerQueue == NULL || statusFeedbackQueue == NULL || stateDoorQueue == NULL)
  {
  ...
  // Ghim các tác vụ MẠNG (WiFi, Firebase) vào Core 1
  xTaskCreatePinnedToCore(
      ketNoiWiFiTask, // Hàm của task
      "KetNoiWiFi",   // Tên task
      4096,           // Stack size
      NULL,           // Tham số
      3,              // Độ ưu tiên cao nhất (để kết nối WiFi trước)
      NULL,           // Task handle
      1);             // Ghim vào Core 1

  xTaskCreatePinnedToCore(
      guiNhanFirebaseGooleSheetTask, // Hàm của task
      "FirebaseHandler",             // Tên task
      12288,                         // Stack size lớn
      NULL,                          // Tham số
      1,                             // Độ ưu tiên
      NULL,                          // Task handle
      1);                            // Ghim vào Core 1

  // Ghim các tác vụ ỨNG DỤNG (RFID, Cảm biến) vào Core 0
  xTaskCreatePinnedToCore(
      quanLyCuaVaRFIDTask, // Hàm của task
      "QuanLyCua",         // Tên task
      6144,                // Stack size (nhiều logic)
      NULL,                // Tham số
      2,                   // Độ ưu tiên (cao hơn cảm biến)
      NULL,                // Task handle
      0);                  // Ghim vào Core 0

  xTaskCreatePinnedToCore(
      kiemTraTuTask, // Hàm của task
      "KiemTraTu",   // Tên task
      4096,          // Stack size
      NULL,          // Tham số
      1,             // Độ ưu tiên
